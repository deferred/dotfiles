...
The output should include "space 'web' already at index 1"
...
Context 'when macOS left two spaces with the same label'
# destroy_excess_spaces cleans duplicates, but it runs last. Until then a
# label query returns two lines, and `[ "$x" -eq ... ]` used to die with
# "integer expression expected", aborting the whole script.
setup_duplicate() {
	SPACES_JSON='[
				{"index":1,"label":"code","display":1},
				{"index":2,"label":"web","display":1},
				{"index":3,"label":"web","display":1}
			]'
	YABAI_SPACE_LABELS=(web code)
}
Before 'setup_duplicate'

It 'uses the first copy and keeps going'
When call reorder_spaces
The status should be success
The stdout should include "moving space 'web' from index 2 to 1"
The result of function commands should include '-m space web --move 1'
End
End
...
Describe 'distribute_spaces_between_displays'
Context 'with two displays'
It 'puts the first five labels on the lower display index'
When call distribute_spaces_between_displays
The status should be success
The output should include 'assigning spaces 1-5 to display 1'
The output should include 'assigning spaces 6-9 to display 2'
End
...
# Two lines of output from the label query used to break the numeric
# comparison and abort the script.
duplicate_label() {
...
When call distribute_spaces_between_displays
The status should be success
The stdout should include "space 'messaging' already on display 2"
The result of function commands should equal ''
End
End
End
...
The status should be success
The output should include 'assigning spaces 1-5 to display 1'
The output should include 'assigning spaces 6-9 to display 1'
The result of function commands should equal ''
End
End
...
The status should be success
The output should include 'preserving space labeled web'
The output should include 'preserving space labeled messaging'
The result of function commands should equal ''
End
End
...
too_few_spaces() {
	...
	YABAI_SPACE_LABELS=(web code)
	NUM_SPACES=2
	# Grow the fixture so the loop terminates.
	yabai() {
		if [ "${2:-}" = "query" ]; then
			case "$3" in
			--spaces) echo "$SPACES_JSON" ;;
		...
		SPACES_JSON='[{"index":1,"label":"web","display":1},{"index":2,"label":"","display":1}]'
	}
}
...
The status should be success
The output should include 'creating space'
